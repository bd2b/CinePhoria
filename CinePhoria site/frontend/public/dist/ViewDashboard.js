...
function rafraichirDashboard() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let chart = null;
        // Charger les stats
        let fullStats = yield DataControllerIntranet.getReservationStatsAll();
        console.log(fullStats);
        if (fullStats.length) {
            console.log('Clés du premier objet :', Object.keys(fullStats[0]));
            console.log('Valeur filmTitre =', fullStats[0].filmTitre);
        }
        ...
        const filmList = document.getElementById("dashboard__listFilms");
        filmList.innerHTML = ""; // reset
        const uniqueTitles = Array.from(new Set(fullStats
            .map((s) => s.filmTitre)
            .filter((titre) => !!titre))).sort((a, b) => a.localeCompare(b, "fr"));
        ...
        function toggleTitleSelection(title, btn) {
        ...
        // Initialisation date
        const dateDiv = document.querySelector(".graph__date");
        if (dateDiv)
            dateDiv.innerHTML = "";
        ...
        function construireListeJours() {
            ...
            const allDates = fullStats.map((s) => s.jour).filter(Boolean).sort();
        ...
        const chartContainer = document.querySelector(".graph__chart");
        ...
        function updateChart() {
            ...
            chart.options.scales.y = { stacked: true, beginAtZero: true };
            chart.update();
        }
