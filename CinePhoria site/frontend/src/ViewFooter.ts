...
console.log(" Version connu ", JSON.stringify(dataController.version))
  ...
  majFooterVersion( 
  dataController.version.MAJEURE?.toString(10) || '',
...
export function majFooterVersion(majStr: string, minStr: string,buildStr: string ) {
  const verstionSpan = document.getElementById("version") as HTMLSpanElement;
  if (verstionSpan) verstionSpan.textContent = `${majStr}.${minStr}(${buildStr})`
}