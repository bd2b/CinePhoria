...
function validateObjectShape(obj: any, shape: Shape): boolean {
    ...
    for (const key in shape) {
        ...
        if (value === undefined || value === null) {
            if (!expected.startsWith('nullable')) return false;
            continue;
        }
 ...
 */
export function validateBodyShape(shape: Shape): RequestHandler {
    return (req, res, next) => {
